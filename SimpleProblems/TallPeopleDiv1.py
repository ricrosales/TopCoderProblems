...
is to return 2 specific heights in a int[]. The first is computed by finding
the shortest person in each row, and then finding the tallest person among
them (the "tallest-of-the-shortest"). The second is computed by finding the
tallest person in each column, and then finding the shortest person among them
...
class TallPeople:
    ...
    def getPeople(people):
        ...
        for i in people:
            r = i.split(' ')
            arr.append(r)
            shortest.append(min(r))

        arr = np.asarray(arr).T.tolist()
        for i in arr:
            tallest.append(max(i))

        return [int(max(shortest)), int(min(tallest))]
